...
_CG.isActive=_CG.isActive+1;if(_CG.isActive>=mObjs.length){getCActive=0;_CG.isActive=0;};selectImage(mObjs[_CG.isActive].id);};var moveLeft=function(){var listH=document.getElementById('listH'),ulC=listH.children;if(getCActive){_CG.isActive=getCActive--;}
_CG.isActive=_CG.isActive-1;if(_CG.isActive<0){_CG.isActive=mObjs.length-1;};selectImage(mObjs[_CG.isActive].id);};var createThumbHighLight=function(id){var imageData=getImageDataById(id);for(var x=0;x<mObjs.length;x++){var getList=document.getElementById('list_'+mObjs[x].id);if(mObjs[x].id==id){getList.setAttribute('class','active');}else{if(getList.className=='active'){getList.removeAttribute('class','active');};};};};var isLocatedBeyondRightEdgeOfView=function(currentLi,ulWidth,sendNo){for(var x=0;x<mObjs.length;x++){if(mObjs[x].id==currentLi){var remainingTillEnd=mObjs.length-currentLi;if(remainingTillEnd<(noThumbsInView-sendNo)){var getLast=Math.min(noThumbsInView-1,mObjs.length-1);newIndexHighlight=((getLast*_CG.thumbs.width)-(ulWidth-_CG.thumbs.width));}}}};var moveHighilightIntoView=function(id){var newIndexHighlight,listH=document.getElementById('listH'),activeLi=getActiveEl(listH),getHWidth=listH.offsetWidth,counter=0;if(activeLi==0){newIndexHighlight=0;}
...
btnNo.onclick=function(){wrapper.removeChild(alertHolder);return false;};};};var confirmationActions=function(action){var listH=document.getElementById('listH'),activeLi=getActiveEl(listH),getCurrentLi=document.getElementById('list_'+activeLi),getCurrentBig=document.getElementById('img_'+activeLi),getHWidth=listH.offsetWidth;if(action=='delete'){var newWidth=(getHWidth-_CG.thumbs.width)-5;listH.style.width=newWidth+'px';getCurrentLi.style.display='none';getCurrentBig.style.display='none';moveRight();}
